...
public sealed class TagDetector : System.IDisposable
{
    ...
    // Overload using real camera intrinsics (fx, fy, cx, cy) for accurate pose estimation
    public void ProcessImage
      (ReadOnlySpan<Color32> image, double fx, double fy, double cx, double cy, float tagSize)
    {
        ImageConverter.Convert(image, _image);
        RunDetectorAndEstimatorWithIntrinsics(fx, fy, cx, cy, tagSize);
    }
    ...
    void RunDetectorAndEstimatorWithIntrinsics
